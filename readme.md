...
Pythonの `socket` モジュールを使用し、特定のローカルポート（デフォルト: 62001）をロックすることで、同一スクリプトの多重起動を防止しています。
...
PyTask,true,scripts/myscript.py,--verbose,60,
BatTask,true,C:\Tools\cleanup.bat,,1440,
...
* **ソケットエラー**: 別のアプリケーションがポート62001を使用している場合、ソースコード内の `LOCK_PORT` を変更してください。
